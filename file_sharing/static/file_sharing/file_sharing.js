...
form_count_field.value = `${access_emails_table.rows.length}`;
let next_form_index = access_emails_table.rows.length;

function addEmail() {    
    let new_row = access_emails_table.insertRow();
    new_row.innerHTML = access_emails_table.rows[0].innerHTML.replaceAll("access_emails-0", `access_emails-${next_form_index}`);
    next_form_index += 1;
    form_count_field.value = `${access_emails_table.rows.length}`
}
